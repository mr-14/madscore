...
import { RaphaelContext } from './raphael-context';
...
export class Renderer {
  ...
  // Set this to true if you're using MadScore inside a runtime
  ...
  constructor(sel, backend) {
    ...
    this.sel = sel;
    if (!this.sel) {
      throw new CommonUtil.RERR('BadArgument', 'Invalid selector for renderer.');
    }
    ...
    if (this.backend === Renderer.Backends.CANVAS) {
      ...
      if (!this.element.getContext) {
        throw new CommonUtil.RERR('BadElement', `Can't get canvas context from element: ${sel}`);
      }
      ...
      throw new CommonUtil.RERR('InvalidBackend', `No support for backend: ${this.backend}`);
    }
  }
  ...
  resize(width, height) {
    if (this.backend === Renderer.Backends.CANVAS) {
      if (!this.element.getContext) {
        throw new CommonUtil.RERR('BadElement', `Can't get canvas context from element: ${this.sel}`);
      }
